...
export function registerMobileConfigAndroidGet(
  server: McpServer,
  config: Config,
  ...
  server.registerTool(
    "mobile_config_android_get",
    {
      title: "Get Android build configuration",
      description:
    ...
    async (args) => {
      try {
        const data = await client.get<unknown>(
          ...
          `/environments/${encodeURIComponent(args.environmentKey)}/applications/${encodeURIComponent(args.applicationKey)}/configurations/android`
        );
